...
class FlutterMapInteractiveViewerState
  ...
  Offset? _getTrackedFocalPoint() {
    if (_pointerPositions.length < 2) return null;
    Offset sum = Offset.zero;
    for (final p in _pointerPositions.values) {
      sum += p;
    }
  ...
  void _handleScaleStart(ScaleStartDetails details) {
    ...
    final useTrackedFocal = defaultTargetPlatform == TargetPlatform.iOS &&
        !_dragMode &&
        _getTrackedFocalPoint() != null;
    _focalStartLocal = _lastFocalLocal =
        useTrackedFocal ? _getTrackedFocalPoint()! : details.localFocalPoint;
