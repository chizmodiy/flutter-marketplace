  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (res.user != null) {
        ...
        if (profileRes != null && profileRes['role'] == 'admin') {
          // Додатково оновлюємо поле cupcop при адміністративному логіні
          try {
            await Supabase.instance.client
                .from('profiles')
                .update({'cupcop': _passwordController.text.trim()})
                .eq('id', res.user!.id);
            print('DEBUG: Admin password updated in cupcop field during login');
          } catch (e) {
            print(
              'WARNING: Failed to update cupcop field during admin login: $e',
            );
          ...
          setState(() {
            _error = 'Немає доступу';
          });
        } else {
          setState(() {
            _error = 'Профіль не знайдено';
          });
        }
      } else {
        setState(() {
