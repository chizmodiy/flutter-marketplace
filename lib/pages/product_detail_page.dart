...
class _ProductDetailPageState extends State<ProductDetailPage> {
  ...
  String? _anonymousUserId; // Add this line
  bool _isFavorite = false;
  Set<String> _favoriteProductIds = {};
  List<Product> _similarProducts = [];
  bool _isSimilarLoading = false;
  String? _similarErrorMessage;
  String _activeContactMethod =
      'phone'; // 'phone', 'whatsapp', 'telegram', 'viber', 'message'
  ...
  void initState() {
    super.initState();
    print(
      'DEBUG (ProductDetailPage): initState called with productId: ${widget.productId}',
    );
  ...
  Future<void> _loadAnonymousUserId() async {
    ...
    if (storedId == null) {
      ...
      print('DEBUG: Generated new anonymous user ID: $storedId');
    } else {
      print('DEBUG: Loaded existing anonymous user ID: $storedId');
    }
  ...
  Future<void> _loadProduct() async {
    try {
      print(
        'DEBUG (ProductDetailPage): Loading product with ID: ${widget.productId}',
      );
      setState(() => _isLoading = true);
      final product = await _productService.getProductById(widget.productId);
      print(
        'DEBUG (ProductDetailPage): Product loaded successfully: ${product.title}',
      );
      final categoryName = await _categoryService.getCategoryNameCached(
        product.categoryId,
      );
      final subcategoryName = await _categoryService.getSubcategoryNameCached(
  ...
  Future<void> _loadSimilarProducts(Product product) async {
    ...
    setState(() {
      ...
      _similarErrorMessage = null;
    });

    try {
      final similar = await _productService.getSimilarProducts(
      ...
      setState(() {
        _isSimilarLoading = false;
        _similarErrorMessage = e.toString();
      });
    }
  }
  ...
  Future<void> _toggleFavorite() async {
    try {
      ...
      if (_isFavorite) {
        ...
        await favoritesService.addToFavorites(widget.productId);
        setState(() {
          _isFavorite = true;
  ...
  Future<void> _toggleSimilarFavorite(Product product) async {
    try {
      final favoritesService = FavoritesService();
      ...
      print('Error toggling favorite for similar product: $e');
    }
  }
  ...
  Widget _buildContactItem(
    String iconPath,
    String text, {
    required bool isSocialIcon,
  }) {
    // Визначаємо тип контакту на основі шляху до іконки
    String contactType = 'phone';
    if (iconPath.contains('whatsapp')) {
      contactType = 'whatsapp';
    } else if (iconPath.contains('telegram')) {
      contactType = 'telegram';
    } else if (iconPath.contains('viber')) {
      contactType = 'viber';
    }
      ...
      child: Row(
        children: [
          SvgPicture.asset(
            ...
            colorFilter: isSocialIcon
                ? null
                : const ColorFilter.mode(Color(0xFFA1A1AA), BlendMode.srcIn),
          ),
          const SizedBox(width: 7),
          GestureDetector(
            onTap: () {
              _launchContact(contactType, text);
            },
            child: Row(
  ...
  Widget _buildContactInfo(String iconPath, String text) {
    ...
    if (iconPath.contains('whatsapp') ||
        iconPath.contains('telegram') ||
        iconPath.contains('viber')) {
      isSocialIcon = true;
    }
  ...
  Future<void> _sendFirstMessage() async {
    if (_currentUserId == null || _product == null || _userProfile == null)
      return;
    ...
    if (chatId == null) {
      ...
      final chatInsert = await client
          .from('chats')
          .insert({'is_group': false, 'listing_id': _product!.id})
          .select()
          .single();
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => ChatDialogPage(
          ...
          listingImageUrl: _product!.photos.isNotEmpty
              ? _product!.photos.first
              : '',
  ...
  Widget build(BuildContext context) {
    print('DEBUG (ProductDetailPage): build method called');
    ...
    return wrapWithKeyboardDismisser(
      Scaffold(
        ...
        floatingActionButton:
            _product != null && _currentUserId != _product!.userId
            ? Container(
                ...
                GestureDetector(
                  ...
                  child: PageView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      // Видаляємо Hero для уникнення конфліктів, особливо при навігації до чату
                      return imageWidget;
                    },
                  ),
                ),
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            _buildNavigationButton(
                              ...
                              onTap: () async {
                                if (_product != null) {
                                  try {
                                    final productUrl =
                                        'https://your-app-url.com/product/${_product!.id}';
                                    await Share.share(
                            ...
                            if (_currentUserId != null &&
                                _product != null &&
                                _currentUserId == _product!.userId) ...[
            ...
            child: Column(
              ...
              children: [
                      ...
                      Wrap(
                        ...
                        children: [
                          ...
                          if (_categoryName != null &&
                            ...
                            _buildTag(_categoryName!, isPrimary: true),
                          // Subcategory Tag
                          if (_subcategoryName != null &&
                              _subcategoryName!.isNotEmpty)
                            _buildTag(_subcategoryName!),
                          // Real Estate Type Tag
                          if (_categoryName == 'Нерухомість' &&
                              _product?.realEstateType != null)
                            _buildTag(
                              _product!.realEstateType == 'sale'
                                  ? 'Продаж'
                                  : 'Оренда',
                            ),
                          // Mileage Tag
                          if (_product?.mileageThousandsKm != null)
                            _buildTag(
                              '${_product!.mileageThousandsKm} тис. км.',
                            ),
                          // Radius R Tag
                          if (_product?.radiusR != null &&
                              _product!.radiusR! > 0)
                            _buildTag('R${_product!.radiusR}'),
                          // Negotiable Tag
                ...
                if (_product!.address != null && _product!.address!.isNotEmpty)
                  GestureDetector(
                    onTap: () =>
                        _launchMapsUrl(_product!.latitude, _product!.longitude),
                    child: Padding(
                ...
                if (_product!.latitude != null &&
                    _product!.longitude != null) ...[
                  SizedBox(
                    height: 364,
                    child: GestureDetector(
                      onTap: () {
                                      ...
                                      IconButton(
                                        ...
                                        onPressed: () {
                                          final currentZoom =
                                              _mapController.zoom;
                                          if (currentZoom < 18) {
                                            // Перевірка максимального масштабу
                                            _mapController.move(
                                              LatLng(
                                                _product!.latitude!,
                                                _product!.longitude!,
                                      ...
                                      IconButton(
                                        ...
                                        onPressed: () {
                                          final currentZoom =
                                              _mapController.zoom;
                                          if (currentZoom > 4) {
                                            // Перевірка мінімального масштабу
                                            _mapController.move(
                                              LatLng(
                                                _product!.latitude!,
                                                _product!.longitude!,
                  ...
                  children: [
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        if (_product?.whatsapp != null &&
                                _product!.whatsapp!.isNotEmpty ||
                            _product?.telegram != null &&
                                _product!.telegram!.isNotEmpty ||
                            _product?.viber != null &&
                                _product!.viber!.isNotEmpty ||
                            _product?.phoneNumber != null &&
                                _product!.phoneNumber!.isNotEmpty)
                          Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              // WhatsApp contact
                ...
                if (_isSimilarLoading && _similarProducts.isEmpty)
                  const Padding(
                    padding: EdgeInsets.symmetric(vertical: 24),
                    child: Center(child: CircularProgressIndicator()),
                  ),
                if (!_isSimilarLoading && _similarProducts.isNotEmpty) ...[
  ...
  Widget _buildTag(String text, {bool isPrimary = false}) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: isPrimary ? const Color(0xFF83DAF5) : const Color(0xFFFAFAFA),
        borderRadius: BorderRadius.circular(16),
      ),
      child: Text(
        text,
        style: TextStyle(
          color: isPrimary ? const Color(0xFF015873) : const Color(0xFF52525B),
          fontSize: 14,
          fontFamily: 'Inter',
          fontWeight: FontWeight.w500,
          height: 1.43,
  ...
  Widget _buildCustomAttributesSection() {
    if (_product?.customAttributes == null ||
        _product!.customAttributes!.isEmpty) {
  ...
  Future<void> _launchContact(String contactType, String contactValue) async {
    String normalizeUaPhone(String raw) {
      String digits = raw.replaceAll(RegExp(r'[^\d+]'), '');
      digits = digits.replaceAll('+', '');
      if (digits.startsWith('380')) return '+$digits';
      if (digits.startsWith('0')) return '+380${digits.substring(1)}';
      return '+380$digits';
    }
    ...
    try {
      ...
      if (type == 'whatsapp') {
        if (contactValue.startsWith('http://') ||
          ...
          await tryLaunch(Uri.parse(contactValue));
          return;
        }
        final e164 = normalizeUaPhone(contactValue); // +380...
        final noPlus = e164.replaceAll('+', '');
        if (await tryLaunch(Uri.parse('whatsapp://send?phone=$noPlus'))) return;
        if (await tryLaunch(Uri.parse('https://wa.me/$noPlus'))) return;
      } else if (type == 'telegram') {
        if (contactValue.startsWith('http://') ||
            contactValue.startsWith('https://')) {
          if (await tryLaunch(Uri.parse(contactValue))) return;
        }
        if (contactValue.trim().startsWith('@')) {
          final username = contactValue.trim().substring(1);
          if (await tryLaunch(Uri.parse('tg://resolve?domain=$username')))
            return;
          if (await tryLaunch(Uri.parse('https://t.me/$username'))) return;
        } else {
          // Treat as phone or username without @
          final cleaned = contactValue.replaceAll(RegExp(r'[^A-Za-z0-9_]'), '');
          if (RegExp(r'^\d{7,}$').hasMatch(cleaned)) {
            final e164 = normalizeUaPhone(cleaned); // +380...
            final noPlus = e164.replaceAll('+', '');
            if (await tryLaunch(Uri.parse('tg://resolve?phone=$noPlus')))
              return;
            // Web fallback for phone is limited; try generic t.me link
            if (await tryLaunch(Uri.parse('https://t.me/$noPlus'))) return;
          } else {
            if (await tryLaunch(Uri.parse('tg://resolve?domain=$cleaned')))
              return;
            if (await tryLaunch(Uri.parse('https://t.me/$cleaned'))) return;
          }
        }
      } else if (type == 'viber') {
        if (contactValue.startsWith('http://') ||
        ...
        if (await tryLaunch(Uri.parse('viber://chat?number=$encoded'))) return;
        // If Viber not available, fallback to call
        final telUri = Uri.parse('tel:${e164.replaceAll('+', '')}');
        if (await tryLaunch(telUri)) return;
      } else if (type == 'phone') {
        final e164 = normalizeUaPhone(contactValue);
        if (await tryLaunch(Uri.parse('tel:$e164'))) return;
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Не вдалося відкрити додаток. Контакт скопійовано.'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Контакт скопійовано в буфер обміну'),
            duration: Duration(seconds: 2),
          ),
        );
      }
        ...
        SizedBox(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final dateText = dateFormatter.format(product.createdAt);
              return Padding(
                ...
                child: SizedBox(
                  ...
                  child: ProductCard(
                    ...
                    imageHeight: 174,
                    onFavoriteToggle: () => _toggleSimilarFavorite(product),
                    onTap: () async {
  ...
  Widget _buildPhotoThumbnails() {
    if (_product == null || _product!.photos.isEmpty) {
      return const SizedBox.shrink();
    }
    ...
    return Padding(
      ...
      child: SizedBox(
        ...
        child: ListView.separated(
          ...
          itemBuilder: (context, index) {
            final photoUrl = _product!.photos[index];
                ...
                child: ClipRRect(
                  ...
                  child: CachedNetworkImage(
                    ...
                    placeholder: (context, url) =>
                        Container(color: const Color(0xFFE4E4E7)),
                    errorWidget: (context, url, error) => Container(
                      color: const Color(0xFFE4E4E7),
                      child: const Icon(
                        Icons.broken_image,
  ...
  Future<void> _startChatWithOwner() async {
    print('DEBUG: _startChatWithOwner called.');
    if (_product == null || _userProfile == null) return;
    final String? initiatorId = _currentUserId ?? _anonymousUserId;
    print(
      'DEBUG: Current User ID: $_currentUserId, Anonymous User ID: $_anonymousUserId, Initiator ID: $initiatorId',
    );
    if (initiatorId == null) {
      print('DEBUG: Initiator ID is null, returning.');
      return; // Should not happen if _anonymousUserId is always set
    }
    final ownerId = _product!.userId;
    print('DEBUG: Product owner ID: $ownerId');
    if (ownerId == initiatorId) {
      print('DEBUG: Initiator is owner, returning.');
      return;
    }
    final client = Supabase.instance.client;
    print('DEBUG: Checking for existing chats...');
    final existingChats = await client
        .from('chats')
        .select('id')
        .eq('listing_id', _product!.id);
    String? chatId;
    print('DEBUG: Existing chats found: ${existingChats.length}');
    if (existingChats.isNotEmpty) {
      // Перевірити, чи обидва користувачі є учасниками
      for (final chat in existingChats) {
        final participants = await client
            ...
            .eq('chat_id', chat['id']);
        final userIds = participants.map((p) => p['user_id'] as String).toSet();
        if (userIds.contains(initiatorId) && userIds.contains(ownerId)) {
          chatId = chat['id'] as String;
          break;
        }
      }
    ...
    if (chatId == null) {
      ...
      final chatInsert = await client
          ...
          .single();
      chatId = chatInsert['id'] as String;
      print('DEBUG: New chat created with ID: $chatId');
      // Додати обох учасників
      await client.from('chat_participants').insert([
        {'chat_id': chatId, 'user_id': initiatorId},
        {'chat_id': chatId, 'user_id': ownerId},
      ]);
      print('DEBUG: Chat participants added.');
    }
    ...
    print('DEBUG: Navigating to ChatDialogPage with chat ID: $chatId');
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => ChatDialogPage(
          chatId: chatId!,
          userName: _currentUserId == null
              ? 'Анонімний користувач'
              : _userProfile!.fullName,
          userAvatarUrl: _currentUserId == null
              ? ''
              : _userProfile!.avatarUrl ?? '',
          listingTitle: _product!.title,
          listingImageUrl: _product!.photos.isNotEmpty
...
class _ComplaintBottomSheetState extends State<ComplaintBottomSheet> {
  ...
  void initState() {
    ...
    widget.descriptionController.addListener(_descriptionListener);
  }
  ...
  @override
  void dispose() {
    widget.descriptionController.removeListener(_descriptionListener);
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final double extraBottomPadding = keyboardVisible ? 200 : 0;
    ...
    return AnimatedPadding(
      ...
      child: SafeArea(
        child: ConstrainedBox(
          constraints: BoxConstraints(
            maxHeight: mediaQuery.size.height * (keyboardVisible ? 0.95 : 0.7),
          ),
          child: Container(
            width: double.infinity,
            decoration: const BoxDecoration(
            ...
            child: SingleChildScrollView(
              padding: EdgeInsets.only(bottom: 24 + extraBottomPadding),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const SizedBox(height: 8),
