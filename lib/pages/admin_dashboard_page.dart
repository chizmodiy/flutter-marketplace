...
import '../widgets/exchange_rates_manager.dart'; // Import the new ExchangeRatesManager widget
      ...
      child: MouseRegion(
        ...
        child: GestureDetector(
          ...
          child: Container(
            ...
            child: SvgPicture.string(svg, width: 20, height: 20, color: color),
          ),
        ),
      ),
    );
...
class _AdminDashboardPageState extends State<AdminDashboardPage> {
  int _selectedTab =
      0; // 0 - Оголошення, 1 - Скарги, 2 - Користувачі, 3 - Курси валют
  ...
  int _currentPage = 1;
  int _totalPages = 1;
  static const int _pageSize = 8; // 8 елементів на сторінку для таблиць
    ...
    try {
      ...
      if (_filterDateFrom != null && _filterDateTo != null) {
        if (_filterDateFrom!.isAfter(_filterDateTo!)) {
          ...
          setState(() {
            _filterErrorMessage =
                'Мінімальна дата не може бути більшою за максимальну';
          });
        } else {
          setState(() {
          ...
          query = query.gte(
            'created_at',
            _filterDateFrom!.toUtc().toIso8601String(),
          );
          ...
          query = query.lte(
            'created_at',
            toInclusive.toUtc().toIso8601String(),
          );
        }
      ...
      if (_filterDateFrom != null &&
          _filterDateTo != null &&
          !(_filterDateFrom!.isAfter(_filterDateTo!))) {
  ...
  Future<void> _fetchComplaints() async {
    ...
    try {
      ...
      setState(() {
        ...
        // Оновлюємо кількість сторінок для вкладки Скарги
        _totalPages = (complaints.length / _pageSize).ceil().clamp(1, 9999);
        // Гарантуємо, що поточна сторінка в межах
      ...
      print(
        '=== _fetchComplaints завершено, знайдено ${complaints.length} скарг, сторінок: $_totalPages ===',
      );
    } catch (e) {
  ...
  Future<void> _fetchUsers() async {
    ...
    try {
      // Читаємо напряму з profiles (RLS безпечне), щоб не ламати відображення
      var query = Supabase.instance.client
          .from('profiles')
          .select(
            'id, first_name, last_name, phone, status, avatar_url, updated_at',
          )
          // Спочатку ті, у кого є first_name (null-и наприкінці)
          .order('first_name', ascending: true, nullsFirst: false)
          // Потім за останнім оновленням (новіші вище)
          .order('updated_at', ascending: false);

      // Примітка: пошук застосуємо на клієнті нижче (див. allUsers.where)

      // Завантажуємо повний список (масштаб малий) і фільтруємо на клієнті
      final users = await query;

      if (users is List && users.isNotEmpty) {
        // debug log перших записів
        // ignore: avoid_print
        print('DEBUG Users sample: ${users.take(3).toList()}');
      }

      // Клієнтське фільтрування пошуку
      ...
      if (_searchQuery.isNotEmpty) {
        ...
        allUsers = allUsers.where((u) {
          ...
          return fn.contains(q) || ln.contains(q) || ph.contains(q);
        }).toList();
      }
      ...
      final pageItems = start < totalCount
          ? allUsers.sublist(start, end.clamp(0, totalCount))
          : <Map<String, dynamic>>[];
        ...
        children: [
                        ...
                        (i) => Padding(
                          ...
                          child: TextButton(
                            onPressed: () {
                              ...
                              // ignore: avoid_print
                              print(
                                'AdminDashboard: switch tab to index $i (${_tabs[i]})',
                              );
                              setState(() => _selectedTab = i);
                              // Підвантажуємо дані відповідної вкладки після перемикання
                              if (_selectedTab == 0) {
                                _fetchProducts();
                              } else if (_selectedTab == 1) {
                                _fetchComplaints();
                            ...
                            style: TextButton.styleFrom(
                              backgroundColor: i == _selectedTab
                                  ? const Color(0xFFF4F4F5)
                                  : Colors.transparent,
                              foregroundColor: i == _selectedTab
                                  ? Colors.black
          ...
          Expanded(
            child: _selectedTab == 0
                        ...
                        child: Column(
                          children: [
                            ...
                            Container(
                              ...
                              // Автоматична висота під контент без жорстких обмежень
                              decoration: BoxDecoration(
                                color: Colors.white,
                              ...
                              child: Column(
                                children: [
                                  ...
                                  if (_isLoadingProducts)
                                    ...
                                    Column(
                                      children: [
                                        for (final ad in _products.take(
                                          _pageSize,
                                        ))
                                          AdminAdTableRow(
                                            ad: ad,
                                            categoryName:
                                                _allCategories[ad.categoryId] ??
                                                '',
                        ...
                        child: Column(
                          children: [
                            ...
                            Container(
                              width: double.infinity,
                              margin: const EdgeInsets.only(bottom: 36),
                              // Без фіксованої висоти — адаптивно до контенту
                              decoration: BoxDecoration(
                                color: Colors.white,
                                borderRadius: BorderRadius.circular(12),
                                border: Border.all(
                                                ...
                                                child: Center(
                                                  child: Column(
                                                    ...
                                                    children: [
                                                      ...
                                                      Text(
                                                        'Коли користувачі створюють скарги, вони з\'являться тут',
                                                        textAlign:
                                                            TextAlign.center,
                                                        style: TextStyle(
                                                          fontSize: 14,
                                                  ...
                                                  onViewDetails: () {
                                                    ...
                                                    showComplaintDialog(
                                                      ...
                                                      onComplaintProcessed: () {
                                                        print(
                                                          '=== onComplaintProcessed викликано ===',
                                                        );
                                                        if (mounted) {
                                                          print(
                                                            '=== Викликаємо _fetchComplaints ===',
                                                          );
                                                          _fetchComplaints();
                                                          print(
                                                            '=== _fetchComplaints завершено ===',
                                                          );
                                                        }
                                                      },
                                                    );
                        ...
                        child: Column(
                          children: [
                            ...
                            Container(
                              width: double.infinity,
                              margin: const EdgeInsets.only(bottom: 36),
                              // Без фіксованої висоти — адаптивний контейнер
                              decoration: BoxDecoration(
                                color: Colors.white,
                                borderRadius: BorderRadius.circular(12),
                                border: Border.all(
                ...
                : _selectedTab ==
                      3 // New tab for Exchange Rates
                ? const ExchangeRatesManager() // We will create this widget next
                : Center(
                    child: Text(
                      'Сторінка: ${_tabs[_selectedTab]}',
  ...
  String _formatPrice(Product ad) {
    ...
    return PriceFormatter.formatCurrency(ad.price!, currency: ad.currency);
  }
  ...
  Future<void> _showFilterDialog() async {
    ...
    String? tempErrorMessage = _filterErrorMessage;
      ...
      builder: (dialogContext) {
        return StatefulBuilder(
          builder: (context, setModalState) {
            Future<void> applyDateFrom() async {
              final now = DateTime.now();
              final today = DateTime(now.year, now.month, now.day);
              final initial = tempDateFrom ?? today;
              // Обмежуємо максимальну дату до сьогодні або до кінцевої дати (якщо вона встановлена)
              final maxDate =
                  (tempDateTo != null && tempDateTo!.isBefore(today))
                  ? tempDateTo!
                  : today;
              final initialDate = initial.isAfter(maxDate)
                  ? maxDate
                  : (initial.isBefore(DateTime(2000))
                        ? DateTime(2000)
                        : initial);
              ...
              if (picked != null) {
                ...
                // Перевірка: початкова дата не може бути більше сьогодні
                if (pickedDate.isAfter(today)) {
                  setModalState(() {
                    tempErrorMessage =
                        'Початкова дата не може бути в майбутньому';
                  });
                  return;
                }
                // Якщо початкова дата більша за кінцеву, автоматично встановлюємо кінцеву на початкову
                if (tempDateTo != null && pickedDate.isAfter(tempDateTo!)) {
                  setModalState(() {
                    tempDateFrom = pickedDate;
                    tempDateTo = pickedDate;
            ...
            Future<void> applyDateTo() async {
              final now = DateTime.now();
              final today = DateTime(now.year, now.month, now.day);
              final initial = tempDateTo ?? tempDateFrom ?? today;
              // Мінімальна дата - початкова дата (якщо встановлена) або 2000
              final minDate = tempDateFrom ?? DateTime(2000);
              final initialDate = initial.isAfter(today)
                  ? today
                  : (initial.isBefore(minDate) ? minDate : initial);
              final picked = await showDatePicker(
                context: context,
                initialDate: initialDate,
                firstDate: minDate.isBefore(DateTime(2000))
                    ? DateTime(2000)
                    : minDate,
                lastDate: today, // Кінцева дата не може бути в майбутньому
              );
              if (picked != null) {
                final pickedDate = DateTime(
                  picked.year,
                ...
                // Перевірка: кінцева дата не може бути більше сьогодні
                if (pickedDate.isAfter(today)) {
                  setModalState(() {
                    tempErrorMessage =
                        'Кінцева дата не може бути в майбутньому';
                  });
                  return;
                }
                // Перевірка: кінцева дата не може бути менше початкової
                if (tempDateFrom != null &&
                    pickedDate.isBefore(tempDateFrom!)) {
                  setModalState(() {
                    tempErrorMessage =
                        'Кінцева дата не може бути менше початкової';
                  });
                  return;
                }
                setModalState(() {
                            ...
                            children: [
                              Expanded(
                                child: _DatePillField(
                                  hintText: 'дд.мм.рр',
                                  value: _formatUiDate(tempDateFrom),
                                  onPick: applyDateFrom,
                                  onChangedText: (_) {},
                                ),
                              ),
                              const SizedBox(width: 8),
                              ...
                              Expanded(
                                child: _DatePillField(
                                  hintText: 'дд.мм.рр',
                                  value: _formatUiDate(tempDateTo),
                                  onPick: applyDateTo,
                                  onChangedText: (_) {},
                                ),
                              ),
                            ...
                            child: Builder(
                              builder: (context) {
                                ...
                                final hasActiveFilters =
                                    ...
                                    tempDateTo != null ||
                                    (tempCategoryId != null &&
                                        tempCategoryId!.isNotEmpty) ||
                                    (tempStatus != null &&
                                        tempStatus!.isNotEmpty);
                                ...
                                return OutlinedButton(
                                  ...
                                  child: Text(
                                    hasActiveFilters ? 'Очистити' : 'Скасувати',
                                    style: const TextStyle(
                                      color: Colors.black,
                                      fontSize: 16,
                                      fontFamily: 'Inter',
                          ...
                          Expanded(
                            child: ElevatedButton(
                              onPressed: () {
                                ...
                                // Перевірка: початкова дата не може бути в майбутньому
                                if (tempDateFrom != null &&
                                    tempDateFrom!.isAfter(today)) {
                                  validationError =
                                      'Початкова дата не може бути в майбутньому';
                                }
                                // Перевірка: кінцева дата не може бути в майбутньому
                                else if (tempDateTo != null &&
                                    tempDateTo!.isAfter(today)) {
                                  validationError =
                                      'Кінцева дата не може бути в майбутньому';
                                }
                                // Перевірка: початкова дата не може бути більше кінцевої
                                else if (tempDateFrom != null &&
                                    tempDateTo != null &&
                                    tempDateFrom!.isAfter(tempDateTo!)) {
                                  validationError =
                                      'Мінімальна дата не може бути більшою за максимальну';
                                }
                                // Перевірка: кінцева дата не може бути менше початкової
                                    ...
                                    tempDateTo != null &&
                                    tempDateTo!.isBefore(tempDateFrom!)) {
                                  validationError =
                                      'Кінцева дата не може бути менше початкової';
                                }
                                ...
                                if (validationError != null) {
                                  setModalState(() {
                                    tempErrorMessage = validationError;
                                  });
                                  setState(() {
                                    _filterErrorMessage = validationError;
                                  });
                                  return;
                                }
    ...
    return Padding(
      ...
      child: Row(
        children: [
              ...
              children: [
                if (ad.photos.isNotEmpty)
                  ...
                  Container(
                    ...
                    height: 60,
                    color: const Color(0xFFE4E4E7),
                  ), // Збільшив розмір placeholder також
                const SizedBox(width: 8),
                Expanded(
                  child: Text(
                    ad.title,
          ...
          Expanded(flex: 2, child: Text(formatDate(ad.createdAt))),
          Expanded(flex: 2, child: Text(formatPrice(ad))),
          Expanded(
            flex: 2,
            child: Text(
              ad.location ?? '',
              ...
              _ActionIconButton(
                ...
                onTap: () {
                  showDeleteListingDialog(
                    ...
                    onDelete: () async {
                      try {
                        await adminService.deleteListing(ad.id);
                        if (context.mounted) {
                          onStatusChanged?.call();
                        }
                      } catch (e) {
...
class ComplaintTableRow extends StatelessWidget {
  ...
  Widget _buildListingImage(dynamic photos) {
    if (photos == null || photos.isEmpty || photos is! List || photos.isEmpty) {
      ...
      return const Icon(Icons.article, size: 24, color: Colors.grey);
    }
...
class UserTableRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    String _sanitizeName(dynamic value) {
      ...
      if (lower == 'null' || lower == 'empty' || lower == 'undefined')
        return '';
      return s;
    }

    final firstName = _sanitizeName(user['first_name']);
    final lastName = _sanitizeName(user['last_name']);
    final String userName = (firstName.isNotEmpty || lastName.isNotEmpty)
        ...
        children: [
          ...
          Expanded(
            ...
            child: Container(
              ...
              child: Text(
                _formatUserDate(user['updated_at']),
                style: const TextStyle(
                  color: Colors.black,
                  fontSize: 16,
                  fontFamily: 'Inter',
...
Future<void> showComplaintDialog({
  ...
  VoidCallback?
  onComplaintProcessed, // New callback for when a complaint is processed
}) async {
  ...
  try {
    final listingResponse = await supabase
        .from('listings')
        .select(
          'id, title, description, photos, price, is_free, location, created_at, status',
        )
        .eq('id', listingId)
        .single();
  ...
  final listingCreatedAt = listing['created_at'] != null
      ? DateTime.tryParse(listing['created_at'])
      : null;
  final listingDate = listingCreatedAt != null
      ? '${listingCreatedAt.day.toString().padLeft(2, '0')} ${_monthUA(listingCreatedAt.month)} ${listingCreatedAt.hour.toString().padLeft(2, '0')}:${listingCreatedAt.minute.toString().padLeft(2, '0')}'
      : '';

  // Дата створення скарги
  final complaintCreatedAt = complaint['created_at'] != null
      ? DateTime.tryParse(complaint['created_at'])
      : null;
  final complaintDate = complaintCreatedAt != null
      ? '${complaintCreatedAt.day.toString().padLeft(2, '0')} ${_monthUA(complaintCreatedAt.month)} ${complaintCreatedAt.hour.toString().padLeft(2, '0')}:${complaintCreatedAt.minute.toString().padLeft(2, '0')}'
      : '';
  ...
  return showDialog(
    ...
    builder: (context) {
      return Dialog(
        ...
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        child: ConstrainedBox(
          constraints: const BoxConstraints(minWidth: 390, maxWidth: 600),
          child: Stack(
            children: [
              Padding(
                padding: const EdgeInsets.fromLTRB(24, 40, 24, 24),
                  ...
                  children: [
                    ...
                    Text(
                      isListingBlocked
                          ...
                          : 'Після підтвердження блокування - це оголошення буде приховане для перегляду користувачами',
                      textAlign: TextAlign.center,
                      style: TextStyle(
                        color: isListingBlocked
                            ? Color(0xFFB42318)
                            : Color(0xFF52525B),
                        fontSize: 14,
                        fontFamily: 'Inter',
                          ...
                          Expanded(
                            child: ElevatedButton(
                              onPressed: () async {
                                ...
                                print(
                                  'onComplaintProcessed callback: $onComplaintProcessed',
                                );
                                ...
                                try {
                                  ...
                                  final supabase = Supabase.instance.client;
                                  print(
                                    'Блокуємо оголошення ${complaint['listing_id']}',
                                  );
                                  await supabase
                                      .from('listings')
                                  ...
                                  if (context.mounted) {
                                    ...
                                    print(
                                      'Context не mounted, не можемо закрити попап',
                                    );
                                  }

                                  print(
                                    'Викликаємо onComplaintProcessed callback',
                                  );
                                  ...
                                  if (context.mounted) {
                                    Navigator.of(context).pop();
                                    print(
                                      'Попап закрито успішно (після помилки)',
                                    );
                                  }
                                  onComplaintProcessed?.call();
                                }
                        ...
                        Expanded(
                          child: Container(
                            ...
                            child: TextField(
                              ...
                              decoration: InputDecoration(
                                hintText:
                                    'Вкажіть причину блокування користувача',
                                hintStyle: TextStyle(
                      ...
                      Expanded(
                        child: ElevatedButton(
                          ...
                          style: ElevatedButton.styleFrom(
                            ...
                            shape: const StadiumBorder(),
                            backgroundColor: isReasonFilled
                                ? const Color(0xFFB42318)
                                : const Color(0xFFD1D5DB),
                            side: BorderSide(
                              color: isReasonFilled
                                  ? const Color(0xFFB42318)
                                  : const Color(0xFFD1D5DB),
                            ),
                            elevation: 0,
