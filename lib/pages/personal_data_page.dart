...
class _PersonalDataPageState extends State<PersonalDataPage> {
    ...
    if (picked != null) {
      ...
      try {
        ...
        if (user == null || session == null || session.isExpired) {
          _showErrorSnackBar('Сесія закінчилася. Будь ласка, увійдіть знову');
        ...
        if (kIsWeb) {
          ...
          try {
            ...
            if (storageError.toString().contains('JWT')) {
              _showErrorSnackBar(
                'Помилка авторизації. Будь ласка, увійдіть знову',
              );
              Navigator.of(context).pushReplacementNamed('/auth');
              return;
            }
        ...
        _showErrorSnackBar('Помилка при завантаженні фото. Спробуйте ще раз');
      } finally {
    ...
    try {
      ...
      if (_tempAvatarUrl != _avatarUrl || _isAvatarDeleted) {
        if (_isAvatarDeleted) {
          ...
          await Supabase.instance.client.auth.updateUser(
            UserAttributes(data: {'avatar_url': _tempAvatarUrl}),
          );
        }
      }
      ...
      if (mounted)
        Navigator.of(
          context,
        ).pop(true); // Повертаємо true щоб показати, що зміни були збережені
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    print(
      'PersonalDataPage.build called, _avatarUrl=$_avatarUrl, _hasCustomAvatar=$_hasCustomAvatar',
    );
    return Scaffold(
                                ...
                                Row(
                                  children: [
                                    ...
                                    GestureDetector(
                                      ...
                                      onTap:
                                          _pickAvatar, // Call _pickAvatar when 'Оновити' is tapped
                                      child: Container(
                                        padding: const EdgeInsets.symmetric(
                                          horizontal: 14,
                                          vertical: 8,
