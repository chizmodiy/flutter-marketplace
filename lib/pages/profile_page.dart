...
class _ProfilePageState extends State<ProfilePage> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Логи скопійовано в буфер обміну')),
        );
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Не вдалося скопіювати логи: $e')),
        );
      }
    }
  }
  ...
  Widget _profileButton({required String text, required VoidCallback onTap}) =>
      GestureDetector(
        onTap: onTap,
        child: Container(
          height: 56,
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
          ...
          body: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                _profileButton(
                  text: 'Скопіювати логи (для підтримки)',
                  onTap: _copyLogs,
                ),
                _profileButton(
                  text: 'Тест',
                ...
                _profileButton(
                  ...
                  onTap: () async {
                    final result = await Navigator.of(context).push(
                      MaterialPageRoute(
                        builder: (context) => ActiveListingsPage(),
                      ),
                    );
                ...
                _profileButton(
                  ...
                  onTap: () async {
                    final result = await Navigator.of(context).push(
                      MaterialPageRoute(
                        builder: (context) => InactiveListingsPage(),
                      ),
                    );
                    // Якщо повернулися з додавання оголошення, оновлюємо сторінку
                    if (result == true) {
                ...
                _profileButton(
                  ...
                  onTap: () async {
                    final result = await Navigator.of(context).push(
                      MaterialPageRoute(
                        builder: (context) => FavoriteListingsPage(),
                      ),
                    );
                    // Якщо повернулися з додавання оголошення, оновлюємо сторінку
                    if (result == true) {
        ...
        Positioned(
          top:
              70, // Позиція аватара - на межі між синім та білим (120 - 34 - 16)
          left: 0,
          right: 0,
          child: Center(
            child: Container(
